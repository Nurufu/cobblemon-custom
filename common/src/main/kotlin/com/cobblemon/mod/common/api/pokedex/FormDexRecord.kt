...
class FormDexRecord {
    companion object {
        val CODEC: Codec<FormDexRecord> = RecordCodecBuilder.create { instance ->
            instance.group(
                ListCodec(Codec.STRING).fieldOf("genders").forGetter { it.genders.map { it.name } },
                ListCodec(Codec.STRING).fieldOf("seenShinyStates").forGetter { it.seenShinyStates.toList() },
