 ...
 */
class SpawnSnowstormEntityParticlePacket(val effectId: Identifier, val entityId: Int, val locator: List<String> = listOf("root")) : NetworkPacket<SpawnSnowstormEntityParticlePacket> {
    companion object {
        val ID = cobblemonResource("spawn_snowstorm_entity_particle")

        fun decode(buffer: PacketByteBuf) = SpawnSnowstormEntityParticlePacket(buffer.readIdentifier(), buffer.readInt(), buffer.readList{buffer.readString()})
    }
